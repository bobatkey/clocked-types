...
$\tyname{Stream}~A = \mu X. A \times \delay\kappa X$, then there is no
way of defining a function $\ident{tail} : \tyname{Stream}~A \to
\tyname{Stream}~A$; the best we can do is $\ident{tail} :
\tyname{Stream}~A \to \delay\kappa\tyname{Stream}~A$. The rest of the
